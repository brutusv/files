#! /bin/bash

if [[ $1 == "test" ]]
then
  PSQL="psql --username=postgres --dbname=worldcuptest -t --no-align -c"
else
  PSQL="psql --username=freecodecamp --dbname=worldcup -t --no-align -c"
fi

# Do not change code above this line. Use the PSQL variable above to query your database.
cat 'games.csv' | while IFS=',' read -r YEAR ROUND WINNER OPPONENT WINNER_GOALS OPPONENT_GOALS 
do
  if [[ $YEAR != 'year' ]]
  then
    # adding teams
    ADD_TEAMS_RESULT=$($PSQL "INSERT INTO teams(name) VALUES('$WINNER');")
    ADD_TEAMS_RESULT=$($PSQL "INSERT INTO teams(name) VALUES('$OPPONENT');")

    # quering winner and opponent ID's
    WINNER_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$WINNER';")
    echo "winner_id = $WINNER_ID"

    OPPONENT_ID=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT';")
    echo "opponent_id = $OPPONENT_ID"

    # adding games
    ADD_GAME_RESULT=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES($YEAR, '$ROUND', $WINNER_ID, $OPPONENT_ID, $WINNER_GOALS, $OPPONENT_GOALS);")
  fi
done