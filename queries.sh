...
echo "$($PSQL "SELECT teams.name FROM games LEFT JOIN teams ON (games.winner_id=teams.team_id) WHERE year='2014' and round='Eighth-Final' UNION SELECT teams.name FROM games LEFT JOIN teams on (games.opponent_id=teams.team_id) WHERE year='2014' and round='Eighth-Final';")"
